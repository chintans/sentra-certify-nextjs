...
import '../assets/svg-font-icon/icon-style.css';
    ...
    <div className="flex flex-col h-screen bg-[#17181d] text-white w-15 p-4">
      ...
      <nav className="flex-1 f-14 w-14">
        ...
        >
          ...
          <span className='icon icon-validate'></span>
          Certificate
        </Link>
        ...
        >
          ...
          <span className='icon icon-setup'></span>
          Settings
        </Link>
        ...
        <Link href="/import" 
          className={`flex items-center gap-3 px-4 py-3 rounded-lg mb-2 opacity-8 hover:text-white opacity-1  ${pathname === '/import' ? 'bg-[#35363f] text-white' : 'text-gray-300 hover:bg-[#2a2a36]'}`}
        >
          {/* <svg className={`w-5 h-5 ${pathname === '/user-roles' ? 'text-[#20c997]' : 'text-gray-300'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" />
          </svg> */}
          <span className='icon icon-path'></span>
          Import Certify
        </Link>
      </nav>
